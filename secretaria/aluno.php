...
switch ($opcao) {
    case 0://inserir novo cadastro de aluno
        $oAluno = new Aluno();
        $s_Senha = $oAluno->gerarSenha();
        break;

    case 1://alterar cadastro de aluno
        $id_aluno = @$_GET['idaluno'];
        if (!$id_aluno) {
            $erro = "ERRO AO EXIBIR ALUNO!<br/>ID ALUNO NAO INFORMADO!";
        } else {
            $oAluno = new Aluno();
            $oAluno->setIdusuario($id_aluno);

            if ($oAluno->listar()) {
                $s_Login = $oAluno->getLogin();
                $s_Senha = $oAluno->getSenha();
                $sNivel_acesso = $oAluno->getNivel_acesso();
                $sNome = $oAluno->getNome();
                $sRua = $oAluno->getRua();
                $iNumero = $oAluno->getNumero();
                $sBairro = $oAluno->getBairro();
                $sMunicipio = $oAluno->getMunicipio();
                $sEstado = $oAluno->getEstado();
                $sCep = $oAluno->getCep();
                $sNacionalidade = $oAluno->getNacionalidade();
                $sUfOrigem = $oAluno->getUf_origem();
                $sPai = $oAluno->getPai();
                $sMae = $oAluno->getMae();
                $iRg = $oAluno->getRg();
                $iCpf = $oAluno->getCpf();
                $sDataCadastro = $oAluno->getData_cadastro();
                $sTelefoneFixo = $oAluno->getTelefone_fixo();
                $sTelefoneCelular = $oAluno->getTelefone_celular();
                $sTelefoneRecado = $oAluno->getTelefone_recado();
                $sEmail = $oAluno->getEmail();
                $iMatricula = $oAluno->getNum_matricula();
                $iTurma = $oAluno->getTurma_idturma();
                $oTurma = new Turma();
                $oTurma->setIdturma($iTurma);
                $oTurma->listar();
                $iCurso = $oTurma->getCurso_idcurso();

                $sRaca = $oAluno->getRaca();
                $sNecessidadesEspeciais = $oAluno->getNecessidades_especiais();

                unset($oAluno);
            } else {
                $erro = "ERRO AO EXIBIR ALUNO!<br/> O ALUNO NAO FOI ENCONTRADO!";
            }
        }
        break;
}
    ...
    <body>		
        <div id='geral'>
            ...
            <div id='painel_exibicao'>
                <form id="formAluno" name="formAluno" action='./manipularAluno.php' method='post' onSubmit='return validarFormulario(this);'>
                    <center><b>Cadastro de Aluno:</b></center>
                    <fieldset>
                        <legend>Nome:</legend>
                    ...
                    <fieldset>
                        ...
                        if ($opcao != 1) {
                            $oAluno = new Aluno();
                            $iMatricula = $oAluno->gerarNumeroMatricula();
                        }
                        ?>
                        N&uacute;mero de matr&iacute;cula:&nbsp;<input type='text' name='campoNumeroMatricula' id='campoNumeroMatricula' class='campoNumeroMatricula' value='<?php echo $iMatricula;?>' disabled/><br/><br/>
                        <input type='hidden' name='ocultoNumeroMatricula' value='<?php echo $iMatricula;?>'/>
                        Ra&ccedil;a:<br/><input type='text' class='campoRaca' name='campoRaca' id='campoRaca' value='<?php echo $opcao ? $sRaca : ""; ?>'/><br/><br/>
                        Necessidades Especiais:<br/><input type='text' class='campoNecessidades' name='campoNecessidades' id='campoNecessidades' value='<?php echo $opcao ? $sNecessidadesEspeciais : ""; ?>'/><br/><br/>
                        Escolha o curso e a turma aos quais o aluno est&aacute; matriculado:<br/><br/>
                        <table>
                            <?php
                            //trecho em php para gerar as opcoes de curso
                            $oCurso = new Curso();
                            $oTurma = new Turma();
                            ...
                            if ($listaDeCursos != NULL) {
                                ...
                                foreach ($listaDeCursos as $curso) {
                                    echo "
                                    <tr>
                                        <td><input type='radio' name='radioCurso' id='radioCurso" . $i . "' onClick='habilitaCurso(\"selectTurma" . $i . "\")' value='" . $curso['idcurso'] . "'/>" . $curso['nome_curso'] . "</td>";

                                    $listaDeTurmas = $oTurma->listarPorCurso($curso['idcurso']);

                                    if ($listaDeTurmas != NULL) {
                                        echo "
                                        <td class='celTurma'>Turma:</td>
                                        <td>
                                            <select class='selectTurma' name='selectTurma' id='selectTurma" . $i . "' disabled>";
                                        foreach ($listaDeTurmas as $turma)
                                            echo "<option value='" . $turma['idturma'] . "' name='" . $turma['nome'] . "'>" . $turma['nome'] . "</option>";
                                        echo "
                                            </select>
                                        </td>
                                    </tr>";
                                        ...
                                        echo "
                                        <td colspan=2><b>NENHUMA TURMA CADASTRADA</b></td></tr>";
                                    }
                                    $i++;
                                }
                            } else {
                                echo "
                                    <tr>
                                            <td><h2>NENHUM CURSO CADASTRADO</h2></td>
                                    </tr>";
                            }
                            ?>
                        </table>
                    ...
                    <fieldset>
                        ...
                        <select name='nivelAcesso' id='nivelAcesso'>
                            ...
                            <?php echo $nivelAcesso == "" ? "<option value='Moderador'>Moderador</option>" : "" ?>
                        </select>
                    </fieldset>

                    <fieldset>
                        <legend>Login:</legend>
                        <input type='text' name='campoLogin' id='campoLogin' readonly value='<?php echo $opcao ? $s_Login : $iMatricula; ?>'/>&nbsp;
                    </fieldset>
                    ...
                    <?php echo $opcao ? "<input type='hidden' name='idAluno' value='$id_aluno'/>" : ""; ?>
                </form>
            </div>
            ...
            if ($opcao == 1 && $erro == "") {
				...
				<script type='text/javascript' language='javascript'>
                    var radioCurso = document.getElementsByName('radioCurso');
                    var selectTurma = null;

                    for (var i=0; i < radioCurso.length; i++){
                         if(radioCurso[i].value == '" . $iCurso . "'){
                            selectTurma = document.getElementById('selectTurma" . $iCurso . "');
                            if(selectTurma != null){
                                radioCurso[i].checked = true;
                                habilitaCurso('selectTurma" . $iCurso . "');
                                for(var j=0; j < selectTurma.length; j++){
                                    if(selectTurma.options[j].value == '" . $iTurma . "'){
                                        selectTurma.selectedIndex = j;
                                        break;
                                    }
                                }
                            }else break;
